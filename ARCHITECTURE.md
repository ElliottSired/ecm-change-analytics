...
The fact table in this notebook follows a transaction fact table design — one row per engineering change event, capturing what happened and when.
A production implementation would receive updated data from the source system on a scheduled basis — daily or weekly depending on the reporting cadence. Each refresh would bring new changes raised, status updates on existing changes, and potentially revised system impact lists as configuration assessments are completed. Rather than replacing the full dataset on each refresh, a production pipeline would append new transaction rows and update existing ones — changes moving from Open to Validated to Closed would generate new status history rows rather than overwriting the original record.
With a full status history implemented — one row per status transition per change with `Status_Date`, `Valid_From`, `Valid_To`, and `Is_Current` columns — the current state of each change would be derived using a window function — `ROW_NUMBER() OVER (PARTITION BY ChangeID ORDER BY Status_Date DESC)` — filtering to rn = 1 to get the most recent status. This pattern combines the analytical richness of a full audit trail with the current-state querying of a transaction table. It would enable time-in-status analysis, rejection and re-raise rate tracking, and validation cycle time measurement — analytical outputs the current implementation cannot produce from the single Status field.